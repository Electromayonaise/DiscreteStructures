...
public class RailwayPopPush {

    private static Scanner reader;

    public static void main(String[] args) {
        reader = new Scanner(System.in);
        RailwayPopPushMethod();
    }

    public static void RailwayPopPushMethod() {

        int n = reader.nextInt();
        while (n != 0) {
            boolean possible = true; // Assume it's possible until proven otherwise
            ...
            while (true) {
                ...
                int coach = reader.nextInt();
                if (coach == 0) {
                    break; // Move to the next train
                }

                station.push(coach);
                for (int i = 1; i < n; i++) {
                    coach = reader.nextInt();
                    station.push(coach);
                }

                Stack<Integer> temp = new Stack<>();
                while (!station.isEmpty()) {
                    if (!station.isEmpty() && !stack.isEmpty() && station.peek() == stack.peek()) {
                        station.pop();
                        stack.pop();
                    } else {
                        temp.push(station.pop());
                    }
                }
                while (!temp.isEmpty()) {
                    station.push(temp.pop());
                }
